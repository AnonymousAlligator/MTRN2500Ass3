...
auto RectPyr::resize_imple(AllAxis const new_size) -> void {length_ = new_size;}

auto RectPyr::rescale_imple(AnyAxis const factor) -> void
{
    length_ = AllAxis{length_.get_value() * factor.get_value()};
}

auto RectPyr::get_colour_imple() const -> Colour {return Colour::black;}

auto RectPyr::set_parent_frame_name_imple(std::string frame_name) -> void
{
    parent_frame_name_ = std::move(frame_name);
}

auto RectPyr::get_location_imple() const -> std::tuple<XAxis, YAxis, ZAxis>
{
    return std::tuple{XAxis{1.0}, YAxis{2.0}, ZAxis{3.0}};
}

auto RectPyr::move_to_imple(XAxis const) -> void {}

auto RectPyr::move_to_imple(YAxis const) -> void {}

auto RectPyr::move_to_imple(ZAxis const) -> void {}

// Move the shape to a new location
auto RectPyr::move_to_imple(XAxis const x, YAxis const y, ZAxis const z) -> void
{
...
auto RectPyr::move_by_imple(YAxis const) -> void {}

auto RectPyr::move_by_imple(ZAxis const) -> void {}

auto RectPyr::move_by_imple(XAxis const, YAxis const, ZAxis const) -> void {}

// Return marker message for displaying the shape
auto RectPyr::get_display_markers_imple()
    -> std::shared_ptr<std::vector<visualization_msgs::msg::Marker>>
{
    return shapes_list_ptr_;
}

auto RectPyr::rotate_about_axis_to_imple(ZAxis radians) -> void {}
auto RectPyr::get_orientation_imple() const -> ZAxis {return ZAxis{0.0};}
auto RectPyr::move_by_imple(XAxis const) -> void {}
} // namespace shapes